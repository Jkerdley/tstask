...
const normalizeData = (
  unnormalizedData: FetchedPostsDTO[]
): NormalizedPosts => {
  const normalizedResult: NormalizedPosts = {
    byId: {},
    allIds: [],
  };
  unnormalizedData.forEach((item) => {
    normalizedResult.byId[item.id] = {
      id: item.id,
      title: item.title,
      body: item.body,
    };
    normalizedResult.allIds.push(item.id);
  });
...
interface NormalizedPosts {
  byId: {
    [key: string]: FetchedPostsDTO;
  };
  allIds: string[];
}

interface FetchedPostsDTO {
  id: string;
  title: string;
  body: string;
}
